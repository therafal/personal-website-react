...
function Typer(props: TyperProps) {
  ...
  const [typingSpeed] = useState<number>(
    props.typingSpeed ? props.typingSpeed : 150
  );
  const [cursor] = useState<string>(props.cursor ? props.cursor : "▌");
  const [typerStarter] = useState<string>(
    props.typerStarter
      ? props.typerStarter
      : "> [#EBCB8B]root[/]@[#A3BE8C]localhost[/]:~$ "
  );
  ...
  useEffect(() => {
    const type = () => {
      if (displayedText.length < text.length) {
        const typingSpeedVariation =
          Math.random() * (typingSpeed - typingSpeed * 0.3) + typingSpeed * 0.3;
        setTimeout(
          () =>
            setDisplayedText((prevText) => prevText + text[prevText.length]),
          typingSpeedVariation
        );
      }
    };
    type();
